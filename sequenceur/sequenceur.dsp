...
sample(0) = soundfile("sample_2 [url:bipsquare_oneshot.wav]", 1);
sample(1) = soundfile("sample_4 [url:Hihat_oneshot_N.wav]", 1);
sample(2) = soundfile("sample_6 [url:Kick_oneshot_N.wav]", 1);
sample(3) = soundfile("sample_8 [url:Snare_oneshot_N.wav]", 1);
...
envelope = en.asr(a,s,r,gate)  : fi.lowpass(1,1) with { //lowpass to prevent clicking 
  ...
  release = + ~ decay;
  sampleDuration = hslider("Decay[acc:0 0 -10 0 10]", 1, 0.02, 1, 0.001) * 44100 : min(44100) : max(4410) : int;
